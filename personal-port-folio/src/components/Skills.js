...
const SkillCard = ({ color, iconPath, title, team, bgColor, textColor, teamTextColor }) => (
  <div className={`relative py-6 px-6 rounded-3xl w-64 my-4 shadow-xl ${bgColor}`}>
    <div
    ...
    <div className="mt-8">
      <p className={`text-xl font-semibold my-2 ${textColor}`}>{title}</p>
      <div className="flex space-x-2 text-gray-400 text-sm">
        ...
        <p className={`${teamTextColor}`}>{team}</p>
      </div>
    </div>
  </div>
);
